...
def compute_per_bin_prod_asym(kpipi_files, ksk_files):
    """
    ...
    """
    files_directory = "/home/lammert/CODE"
    ...
    for w in range(2):
        if w == 0:
            ...
            weights_file = f"{files_directory}/{strat}/WEIGHTING/{w}_iter/weights_kpipi_{w}.root"
        else : 
            print(len(ksk_files))
            files = ksk_files
            weights_file = f"{files_directory}/{strat}/WEIGHTING/{w}_iter/weights_ksk_{w}.root"
        ...
        for ff,fil in enumerate(files) : 
            ...
            file = ROOT.TFile.Open(files_directory + "/Dalitz_Bins_Kp_TOS/" + fil)
